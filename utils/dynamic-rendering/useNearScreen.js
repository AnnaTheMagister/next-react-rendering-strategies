...
const hasIntersectionObserverSupport = () =>
  typeof window.IntersectionObserver !== "undefined"; // import intersection observer polyfill only if on client
...
export default ({ ref }) => {
  ...
  useEffect(
    function() {
      ...
      getObserver().then(observer => observer.observe(current)); // mutate the current target to add a method to be executed when intersecting
